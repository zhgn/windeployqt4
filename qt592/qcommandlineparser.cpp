...
{
    QCommandLineOption opt(QStringList() << QString::fromLatin1("v") << QString::fromLatin1("version"), tr("Displays version information."));
    addOption(opt);
...
QCommandLineOption QCommandLineParser::addHelpOption()
{
    QCommandLineOption opt(QStringList()
                ...
                << QString::fromLatin1("help"), tr("Displays this help."));
    addOption(opt);
...
QString QCommandLineParser::errorText() const
{
    ...
    if (d->unknownOptionNames.count() > 1)
        return tr("Unknown options: %1.").arg(d->unknownOptionNames.join(QString::fromLatin1(", ")));
    return QString();
}
...
void QCommandLineParser::process(const QStringList &arguments)
{
    ...
    if (d->builtinVersionOption && isSet(QString::fromLatin1("version")))
        showVersion();

    if (d->builtinHelpOption && isSet(QString::fromLatin1("help")))
...
bool QCommandLineParserPrivate::parse(const QStringList &args)
{
    ...
    const QString     doubleDashString(QString::fromLatin1("--"));
...
static QString wrapText(const QString &names, int longestOptionNameString, const QString &description)
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        if (breakAt != -1) {
            ...
            text += description.mid(lineStart, numChars) + QChar::fromLatin1('\n');
            x = 0;
            lastBreakable = -1;
...
QString QCommandLineParserPrivate::helpText() const
{
    ...
    QString text;
    QString usage;
    usage += QCoreApplication::instance()->arguments().first(); // executable name
