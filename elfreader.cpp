...
{
    return QString::fromLatin1("'%1' is an invalid ELF object (%2)")
...
ElfReader::Result ElfReader::readIt()
{
    ...
    if (fdlen < 64) {
        m_errorString = QString::fromLatin1("'%1' is not an ELF object (file too small)").arg(QDir::toNativeSeparators(m_binary));
        return NotElf;
    }

    if (strncmp(mapper.start, "\177ELF", 4) != 0) {
        m_errorString = QString::fromLatin1("'%1' is not an ELF object").arg(QDir::toNativeSeparators(m_binary));
        return NotElf;
    }
    ...
    if (m_elfData.elfclass != Elf_ELFCLASS32 && m_elfData.elfclass != Elf_ELFCLASS64) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("odd cpu architecture"));
        return Corrupt;
    }
    ...
    //     if (errorString)
    //         *errorString = QLibrary::QString::fromLatin1("'%1' is an invalid ELF object (%2)")
    ...
    if (m_elfData.endian != Elf_ELFDATA2LSB && m_elfData.endian != Elf_ELFDATA2MSB) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("odd endianness"));
        return Corrupt;
    }
    ...
    if (e_shsize > fdlen) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("unexpected e_shsize"));
        return Corrupt;
    }

    quint32 e_phentsize = getHalfWord(data, m_elfData);
    if (e_phentsize != (is64Bit ? 56 : 32)) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("invalid structure"));
    ...
    if (e_shentsize % 4) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("unexpected e_shentsize"));
        return Corrupt;
    }
    ...
    if (data != mapper.ustart + (is64Bit ? 64 : 52)) {
        m_errorString = msgInvalidElfObject(m_binary, QString::fromLatin1("unexpected e_phentsize"));
        return ElfReader::Corrupt;
    }

    if (quint64(e_shnum) * e_shentsize > fdlen) {
        const QString reason = QString::fromLatin1("announced %1 sections, each %2 bytes, exceed file size").arg(e_shnum).arg(e_shentsize);
...
//        m_errorString = QLibrary::QString::fromLatin1("'%1' is an invalid ELF object (%2)")
//           .arg(m_binary)
...
//    }
    ...
    if (e_shoff) {
        ...
        if (quint32(stringTableFileOffset + e_shentsize) >= fdlen
            ...
            const QString reason = QString::fromLatin1("string table seems to be at 0x%1").arg(soff, 0, 16);
            m_errorString = msgInvalidElfObject(m_binary, reason);
            return Corrupt;
        }
        ...
        for (quint32 i = 0; i < e_shnum; ++i) {
            ...
            if (stringTableFileOffset + sh.index > fdlen) {
                const QString reason = QString::fromLatin1("section name %1 of %2 behind end of file")
                                       .arg(i).arg(e_shnum);
                m_errorString = msgInvalidElfObject(m_binary, reason);
                return Corrupt;
            }
...
QList<QByteArray> ElfReader::dependencies()
{
    ...
    ElfMapper mapper(this);
    if (!mapper.map()) {
        m_errorString = QString::fromLatin1("Mapper failure");
        return result;
    }
    ...
    foreach (const ElfSectionHeader &eh, headers) {
        if (eh.name  == QByteArray(".dynstr")) {
            dynStrOffset = eh.offset;
        } else if (eh.name  == QByteArray(".dynamic")) {
    ...
    if (!dynStrOffset || !dynamicOffset) {
        m_errorString = QString::fromLatin1("Not a dynamically linked executable.");
        return result;
    }
