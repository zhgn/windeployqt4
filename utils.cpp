...
#include "qt592/qstandardpaths.h"
#if defined(Q_OS_WIN)
...
bool createSymbolicLink(const QFileInfo &source, const QString &target, QString *errorMessage)
{
    ...
    if (!QDir::setCurrent(source.absolutePath())) {
        *errorMessage = QString::fromLatin1("Unable to change to directory %1.").arg(QDir::toNativeSeparators(source.absolutePath()));
        return false;
    }
                ...
                QString *errorMessage)
{
    ...
    if (stdOut) {
        ...
        if (si.hStdOutput == INVALID_HANDLE_VALUE) {
            if (errorMessage)
                *errorMessage = QString::fromLatin1("Error creating stdout temporary file");
            return false;
        }
    ...
    if (stdErr) {
        ...
        if (si.hStdError == INVALID_HANDLE_VALUE) {
            if (errorMessage)
                *errorMessage = QString::fromLatin1("Error creating stderr temporary file");
            return false;
        }
        si.dwFlags |= STARTF_USESTDHANDLES;
    }
    ...
    if (!CreateProcessW(0, commandLineW.data(), 0, 0, /* InheritHandles */ TRUE, 0, 0,
        ...
        if (errorMessage)
            *errorMessage = QString::fromLatin1("CreateProcessW failed: ") + winErrorMessage(GetLastError());
        return false;
    }
...
static inline char *tempFilePattern()
{
    ...
    if (!path.endsWith(QLatin1Char('/')))
        path += QLatin1Char('/');
    path += QString::fromLatin1("tmpXXXXXX");
                ...
                QString *errorMessage)
{
    ...
    if (stdOut) {
        ...
        if (stdOutFile < 0) {
            *errorMessage = QString::fromLatin1("mkstemp() failed: ") + QString::fromLocal8Bit(strerror(errno));
            return false;
        }
    }
    ...
    if (pID < 0) {
        *errorMessage = QString::fromLatin1("Fork failed: ") + QString::fromLocal8Bit(strerror(errno));
        return false;
    }
    ...
    if (waitResult < 0) {
        *errorMessage = QString::fromLatin1("Wait failed: ") + QString::fromLocal8Bit(strerror(errno));
        return false;
    }
    if (!WIFEXITED(status)) {
        *errorMessage = binary + QString::fromLatin1(" did not exit cleanly.");
        return false;
    }
    if (exitCode)
...
QMap<QString, QString> queryQMakeAll(QString *errorMessage)
{
    ...
    const QString binary = QString::fromLatin1("qmake");
    if (!runProcess(binary, QStringList(QString::fromLatin1("-query")), QString(), &exitCode, &stdOut, &stdErr, errorMessage))
        return QMap<QString, QString>();
    if (exitCode) {
        *errorMessage = binary + QString::fromLatin1(" returns ") + QString::number(exitCode)
    ...
    QFile qconfigPriFile(result.value(QString::fromLatin1("QMAKE_MKSPECS")) + QString::fromLatin1("/qconfig.pri"));
...
QString queryQMake(const QString &variable, QString *errorMessage)
{
    ...
    const QString binary = QString::fromLatin1("qmake");
    QStringList args;
    args << QString::fromLatin1("-query ") << variable;
                ...
                const QString &targetDirectory, unsigned flags, JsonOutput *json, QString *errorMessage)
{
    ...
    if (sourceFileInfo.isDir()) {
        ...
        foreach (const QFileInfo &entryFi , allEntries) {
                       ...
                       bool *isDebug)
{
    ...
    if (data.sectionHeaders.isEmpty()) {
        *errorMessage = QString::fromLatin1("Unable to read ELF binary \"")
            + QDir::toNativeSeparators(elfExecutableFileName) + QString::fromLatin1("\": ")
    ...
    if (dependentLibraries) {
        ...
        if (libs.isEmpty()) {
            *errorMessage = QString::fromLatin1("Unable to read dependenices of ELF binary \"")
                + QDir::toNativeSeparators(elfExecutableFileName) + QString::fromLatin1("\": ")
                + elfReader.errorString();
                return false;
        }
        ...
        if (optVerboseLevel > 1) {
            ...
            if (dependentLibrariesIn) {
                ...
                if (optVerboseLevel > 2)
                    std::wcout << dependentLibrariesIn->join(QChar::fromLatin1(' '));
                else
...
{
    const QString prefix = QString::fromLatin1("D3Dcompiler_");
    ...
    if (!kitDir.isEmpty()) {
        QString redistDirPath = QDir::cleanPath(kitDir) + QString::fromLatin1("/Redist/D3D/");
        if (platform & ArmBased) {
            redistDirPath += QString::fromLatin1("arm");
        } else {
            redistDirPath += wordSize == 32 ? QString::fromLatin1("x86") : QString::fromLatin1("x64");
        }
...
bool removeRecursively(QDir dir)
{
    ...
    while (di.hasNext()) {
        ...
        if (fi.isDir() && !fi.isSymLink()) {
            ok = removeRecursively(QDir(filePath)); // recursive
        } else {
